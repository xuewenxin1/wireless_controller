...
extern void Modbus_SetDefrostDisplay(u16 on);
...
void	Temp_Limit(unsigned short Addr,unsigned char TempType)
{
	...
	{
		lo = (unsigned short)TempUnitTrans(SET_TEMP_MODE1_C_LO, 'F');
		hi = (unsigned short)TempUnitTrans(SET_TEMP_MODE1_C_HI, 'F');
	}
	else if(Addr == 0x481a)
	{
		lo = (unsigned short)TempUnitTrans(SET_TEMP_MODE2_C_LO, 'F');
		hi = (unsigned short)TempUnitTrans(SET_TEMP_MODE2_C_HI, 'F');
	}
	else if(Addr == 0x481c)
	{
		lo = (unsigned short)TempUnitTrans(SET_TEMP_MODE3_C_LO, 'F');
		hi = (unsigned short)TempUnitTrans(SET_TEMP_MODE3_C_HI, 'F');
	}
	else
		return;
