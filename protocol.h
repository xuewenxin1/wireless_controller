...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "joiygnz23r1abs3g"    //开发平台创建产品后生成的16位字符产品唯一标识

#define MCU_VER "1.0.0"         //用户的软件版本,用于MCU固件升级,MCU升级版本需修改
//extern unsigned char ver[6];
...
#define WIFIR_UART_SEND_BUF_LMT         160             //DP105=75B/历史故障125B，原48会溢出
/******************************************************************************
                        4:定义模块工作方式
模块自处理:
...
//外机模式(可下发可上报)
//备注:life_water:生活热水
//underfloor_heat：地暖系统
//swimming_pool：泳池控制
//power_on：关闭
#define DPID_MODE 2
//童锁开关(可下发可上报)
//备注:
#define DPID_CHILD_LOCK 3
//目标水温(可下发可上报)
//备注:
#define DPID_TEMP_SET 4
//温标切换(可下发可上报)
//备注:
#define DPID_TEMP_UNIT_CONVERT 6
//除霜(可下发可上报)
//备注:
#define DPID_DEFROST 7
//水箱温度(只上报)
//备注:
#define DPID_TEMP_CURRENT 16
//首页-内机模式(只上报)
//备注:auto：自动
//cool：制冷
//hot：制热
//wet：除湿
//wind：送风
#define DPID_WORK_STATE 17
//进水温度(只上报)
//备注:
#define DPID_TEMP_TOP 21
//泳池温度(只上报)
//备注:
#define DPID_EFFLUENT_TEMP 25
//室外温度(只上报)
//备注:
#define DPID_AROUND_TEMP 26
//上电状态设置(可下发可上报)
//备注:
#define DPID_RELAY_STATUS 101
//设备时间-时间戳(可下发可上报)
//备注:
#define DPID_TIMESTAMP 102
//首页故障信息(只上报)
//备注:0代表无故障
#define DPID_FAULT_VALUE 103
//内机(可下发可上报)
//备注:不固定长度，每组代表一个内机，每组6字节；
//1.内机编号；
//2.单台内机开关：0x00: 关闭，0x01: 开启；
//3.室内温度：16~32℃；
//4.运行模式：0:自动, 1:制冷, 2:制热, 3:送风, 4:除湿 ；
//5.风速：0:自动, 1:低风, 2:中风, 3:高风, 4:超高风；
//6.设置温度：16~32℃；
#define DPID_INDOOR_UNIT 104
//状态查询(只上报)
//备注:
#define DPID_CHECK_STATUS 105
//电量统计(只上报)
//备注:16 字节，按顺序包含4个float32: [市电功率，市电今日累计，光伏功率，光伏今日累计]
//0-3：市电功率，
//4-7：市电今日累计，
//8-11：光伏功率，
//12-15：光伏今日累计。
//举例：33 33 1D 40 CD CC 7C 41 C3 F5 47 40 CD CC 46 41
//0x33331D40= 2.45(市电功率)
//0xCDCC7C41= 15.8(市电今日累计)
//0xC3F54740= 3.12(光伏功率)
//0xCDCC4641= 12.4(光伏今日累计)
#define DPID_ELECTRICITY_STATISTICS 106
//历史故障清空(只下发)
//备注:true:清空历史故障
#define DPID_HISTORY_FAULT_EMPTY 107
//蒸发器温度(只上报)
//备注:mcu上报整数，面板获取后除以10保留一位小数；如mcu上报142，面板获取后142/10 =14.2 ℃
#define DPID_EVAPORATOR_TEMP 108
//除霜频率(只上报)
//备注:
#define DPID_DEFROST_FREQUENCY 109
//除霜退出T3温度(只上报)
//备注:
#define DPID_DEFROST_OUT_TEMP 110
//除霜时间(只上报)
//备注:
#define DPID_DEFROST_TIME 111
//历史故障1(只上报)
//备注:数据结构为 25 条记录，每条记录固定 5 字节：
//字节 0：故障代码(uint8_t)，0：无故障，1~64：有故障；
//字节 1-4：时间戳(uint32_t, 小端序)；
//整个数据块总长为 125 字节。
#define DPID_HISTORY_FAULT_1 112
//历史故障2(只上报)
//备注:数据结构为 25 条记录，每条记录固定 5 字节：
//字节 0：故障代码(uint8_t)，0：无故障，1~64：有故障；
//字节 1-4：时间戳(uint32_t, 小端序)；
//整个数据块总长为 125 字节。
#define DPID_HISTORY_FAULT_2 113
//历史故障3(只上报)
//备注:数据结构为 25 条记录，每条记录固定 5 字节：
//字节 0：故障代码(uint8_t)，0：无故障，1~64：有故障；
//字节 1-4：时间戳(uint32_t, 小端序)；
//整个数据块总长为 125 字节。
#define DPID_HISTORY_FAULT_3 114
//历史故障4(只上报)
//备注:数据结构为 25 条记录，每条记录固定 5 字节：
//字节 0：故障代码(uint8_t)，0：无故障，1~64：有故障；
//字节 1-4：时间戳(uint32_t, 小端序)；
//整个数据块总长为 125 字节。
#define DPID_HISTORY_FAULT_4 115
//内机定时开机(可下发可上报)
//备注:固定长度 8 字节
#define DPID_INSIDE_TIME_OPEN 116
//外机定时开机(可下发可上报)
//备注:固定 6 字节
#define DPID_EXTERNAL_TIME_OPEN 117
//内机定时关机(可下发可上报)
//备注:固定3字节
#define DPID_INSIDE_TIME_CLOSE 118
//外机定时关机(可下发可上报)
//备注:固定3字节
#define DPID_EXTERNAL_TIME_CLOSE 119
...
void protocol_reset_dp_dedup(void);
